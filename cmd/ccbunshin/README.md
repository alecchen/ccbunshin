...
Bash reads `$BASH_COMPLETION_USER_DIR/completions/` (default
`~/.local/share/bash-completion/completions/`) at startup:

```sh
ccbunshin completion bash > "${BASH_COMPLETION_USER_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/bash-completion}/completions/ccbunshin"
```

macOS's stock `/usr/bin/bash` does not read that directory; save the script
anywhere and `source` it from `~/.bashrc` instead.
...
Sourcing registers both completions. A file on `fpath` instead (with
`fpath=(~/.zsh/completions $fpath)` before `compinit`) covers `ccbunshin` only:
`compinit` reads the file's `#compdef` line and never runs the rest, so the
`claude` completion stays unregistered.
