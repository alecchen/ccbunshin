...
Bash has no completion directory of its own, so source the script from
`~/.bashrc`:
...
Where the separate `bash-completion` package is installed, the script can go in
`$BASH_COMPLETION_USER_DIR/completions/` (default
`~/.local/share/bash-completion/completions/`) instead, loaded on demand. It must
be named `ccbunshin.bash` there - that package loads `<command>.bash` and ignores
other names - and it only does so after it has itself been sourced.

For zsh, source the script from `~/.zshrc` after `compinit` has run:
...
Sourcing registers both completions and works from any path. A file on `fpath`
