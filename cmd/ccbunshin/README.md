...
`ccbunshin completion <bash|zsh>` prints a completion script for the CLI, and for
zsh also for the project-aware `claude` wrapper. It is generated by the binary
rather than shipped as a file, so the command surface cannot drift from the CLI.

Nothing is installed for you, and no directory has to be created: write the
script to a file in your home directory and source it from the rc file your shell
actually reads.
...
| bash | `~/.bash_profile` | macOS default: a terminal bash is a login shell and reads no `~/.bashrc` |
...
Both shells have directories that load a completion script with no rc line, if you
would rather use one.

**bash** - the on-demand directory belongs to the separate `bash-completion`
package, which many macOS installs do not have:
...
The file must be named `ccbunshin.bash` there - the package loads `<command>.bash`
on demand and ignores other names - and it reads nothing until it has itself been
sourced, so this route needs an rc line too. System locations work where the
package is installed system-wide: `/usr/share/bash-completion/completions`
(Debian/Ubuntu), `/opt/homebrew/share/bash-completion/completions` (macOS),
normally root-owned.

**zsh** - any directory already on `$fpath` (`print -l $fpath`), typically
`/usr/share/zsh/site-functions`, `/opt/homebrew/share/zsh/site-functions` on
Homebrew, and `~/.oh-my-zsh/custom/completions` under oh-my-zsh, saved as
`_ccbunshin`. It completes `ccbunshin` but not `claude`: `compinit` reads the
file's leading `#compdef` line and never runs the rest, so the wrapper's
completion still needs the `source` line.
...
`tests/completion-test.sh` builds the binary and drives both activation paths for
both shells, and checks that the command lists inside the scripts cover the CLI.
