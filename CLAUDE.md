...
13. A client abort is a `warn` (not an `error`), and the provider timeout defaults to `5m`.
14. Release bodies are the commit subjects in the tag range, generated by `tests/release-notes.sh`, not by GitHub.
